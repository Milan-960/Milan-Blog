...
const MobileNav = () => {
  ...
  const onToggleNav = () => {
    setNavShow((status) => {
      if (status) {
        if (navRef.current) enableBodyScroll(navRef.current); // Ensure navRef.current is not null
      } else {
        if (navRef.current) disableBodyScroll(navRef.current); // Ensure navRef.current is not null
      }
      return !status;
    });
  };
      ...
      <Transition appear show={navShow} as={Fragment} unmount={false}>
        <Dialog as="div" onClose={onToggleNav} unmount={false}>
          ...
          <Transition.Child
            ...
            unmount={false}
          >
            <Dialog.Panel className="fixed left-0 top-0 z-[99] h-full w-full bg-white opacity-95 duration-300 dark:bg-gray-950 dark:opacity-[0.98]">
              <nav
                ref={navRef}
